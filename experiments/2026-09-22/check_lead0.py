...
def main():
    ...
    # We'll examine page 22 and page 47 (0-indexed blocks)
    p22_lengths = word_lengths_from_block(blocks[22])
    p47_lengths = word_lengths_from_block(blocks[47])
    print(f"Page 22 word lengths: {p22_lengths}")
    print(f"Page 47 word lengths: {p47_lengths}")
    ...
    print(f"Target {target} found in page 22 at word index {idx22}")
    print(f"Target {target} found in page 47 at word index {idx47}")
    if idx22 != -1 and idx47 != -1:
        ...
        for offset in range(8):
            ...
            print(f"  Word {widx}: runes='{rune}' values={vals} len={len(vals)}")
        print("\nPage 47 span:")
        for offset in range(8):
            widx = idx47 + offset
